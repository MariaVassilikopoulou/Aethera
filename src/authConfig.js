...
const NEXT_PUBLIC_AZURE_KNOWNAUTHORITY_ENV = process.env.NEXT_PUBLIC_AZURE_KNOWNAUTHORITY;
const NEXT_PUBLIC_API_SCOPE_URI_ENV = process.env.NEXT_PUBLIC_API_SCOPE_URI;

if (!NEXT_PUBLIC_AZURE_CLIENT_ID_ENV || !NEXT_PUBLIC_AZURE_AUTHORITY_ENV || 
  !NEXT_PUBLIC_AZURE_KNOWNAUTHORITY_ENV || !NEXT_PUBLIC_API_SCOPE_URI_ENV) {
  
  // Provide a clear list of missing variables for easy debugging
  const missingVars = [];
  if (!NEXT_PUBLIC_AZURE_CLIENT_ID_ENV) missingVars.push("NEXT_PUBLIC_AZURE_CLIENT_ID");
  if (!NEXT_PUBLIC_AZURE_AUTHORITY_ENV) missingVars.push("NEXT_PUBLIC_AZURE_AUTHORITY");
  if (!NEXT_PUBLIC_AZURE_KNOWNAUTHORITY_ENV) missingVars.push("NEXT_PUBLIC_AZURE_KNOWNAUTHORITY");
  if (!NEXT_PUBLIC_API_SCOPE_URI_ENV) missingVars.push("NEXT_PUBLIC_API_SCOPE_URI");

  throw new Error(`MSAL Configuration Error: Missing required environment variables: ${missingVars.join(', ')}. Please check your .env file and server restart.`);
}
